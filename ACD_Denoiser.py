...
class ACD_Denoiser(nn.Module):
    ...
    def forward(self,
        ...
        x = self.layer_norm_mid(x) # x torch.Size([5, 185, 512]) 
        x = self.output_layer_mid(x) # x torch.Size([5, 185, 350])
        
        # 수정 부분
        o_reshaped = x.view(x.shape[0], x.shape[1], 177, 7)
        o_1, o_2 = torch.split(o_reshaped, [3, 4], dim=-1)
        o_1 = o_1.reshape(o_1.shape[0], o_1.shape[1], 177 * 3)
        o_2 = o_2.reshape(o_2.shape[0], o_2.shape[1], 177 * 4)
