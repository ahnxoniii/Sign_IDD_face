...
NUM_JOINTS = 177
TRG_SIZE = NUM_JOINTS * 3

BODY_JOINTS = 49
BODY_DIM = BODY_JOINTS * 3      # 147
FACE_JOINTS = 128
FACE_DIM = FACE_JOINTS * 3              # 384
...
class Loss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        ####### 수정 부분 
        pred_body = preds_masked[:, :, :BODY_DIM]
        tgt_body = targets_masked[:, :, :BODY_DIM]

        pred_face = preds_masked[:, :, BODY_DIM:TRG_SIZE]
        tgt_face = targets_masked[:, :, BODY_DIM:TRG_SIZE]

        body_loss = self.criterion(pred_body, tgt_body)
        face_loss =  self.criterion_face(pred_face, tgt_face)
        ...
        # 수정 부분
        preds_masked_length = preds_masked_length * loss_mask[:, :, :177]
        targets_masked_length = targets_masked_length * loss_mask[:, :, :177]
        preds_masked_direct = preds_masked_direct * loss_mask[:, :, :177*3]
        targets_masked_direct = targets_masked_direct * loss_mask[:, :, :177*3]

        # Calculate loss just over the masked predictions
        loss = body_loss + 1 * face_loss  + 0.1 * self.criterion_bone(preds_masked_direct, targets_masked_direct)
...
def get_length_direct(trg):
    # 수정 부분
    trg_reshaped = trg.view(trg.shape[0], trg.shape[1], 177, 3)
